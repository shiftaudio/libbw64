    ...
    uint64_t numberOfSamples, uint16_t bitsPerSample)
{
    ...
    if (bitsPerSample == 16) {
        for (uint64_t i = 0; i < numberOfSamples; ++i) {
            int16_t sampleValue = (inBuffer[i * bytesPerSample + 1] & 0xff) << 8 | (inBuffer[i * bytesPerSample] & 0xff);
            outBuffer[i] = sampleValue / 32768.f;
        }
    } else if (bitsPerSample == 24) {
        for (uint64_t i = 0; i < numberOfSamples; ++i) {
            int32_t sampleValue = (inBuffer[i * bytesPerSample + 2] & 0xff) << 24 | (inBuffer[i * bytesPerSample + 1] & 0xff) << 16 | (inBuffer[i * bytesPerSample] & 0xff) << 8;
            outBuffer[i] = sampleValue / 2147483647.f;
        }
    } else if (bitsPerSample == 32) {
        for (uint64_t i = 0; i < numberOfSamples; ++i) {
            int32_t sampleValue = (inBuffer[i * bytesPerSample + 3] & 0xff) << 24 | (inBuffer[i * bytesPerSample + 2] & 0xff) << 16 | (inBuffer[i * bytesPerSample + 1] & 0xff) << 8 | (inBuffer[i * bytesPerSample] & 0xff);
            outBuffer[i] = sampleValue / 2147483647.f;
        }
    } else {
    ...
    uint64_t numberOfSamples, uint16_t bitsPerSample)
{
    ...
    if (bitsPerSample == 16) {
        for (uint64_t i = 0; i < numberOfSamples; ++i) {
            ...
            int16_t sampleValue = static_cast<int16_t>(sampleValueClipped * 32767.);
        ...
        for (uint64_t i = 0; i < numberOfSamples; ++i) {
            auto sampleValueClipped = clipSample(inBuffer[i]);
            int32_t sampleValue = static_cast<int32_t>(sampleValueClipped * 8388607.);
            outBuffer[i * bytesPerSample] = sampleValue & 0xff;
            outBuffer[i * bytesPerSample + 1] = (sampleValue >> 8) & 0xff;
        ...
        for (uint64_t i = 0; i < numberOfSamples; ++i) {
            auto sampleValueClipped = clipSample(inBuffer[i]);
            int32_t sampleValue = static_cast<int32_t>(sampleValueClipped * 2147483647.);
            outBuffer[i * bytesPerSample] = sampleValue & 0xff;
            outBuffer[i * bytesPerSample + 1] = (sampleValue >> 8) & 0xff;
            outBuffer[i * bytesPerSample + 2] = (sampleValue >> 16) & 0xff;
