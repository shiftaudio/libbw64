...
public:
        ...
        std::shared_ptr<ExtraData> extraData = nullptr)
    {
        ...
        if (bitsPerSample_ != 16u && bitsPerSample_ != 24u && bitsPerSample_ != 32u) {
...
public:
    ...
    {
        return trackIndex() == rhs.trackIndex() && uid() == rhs.uid() && trackRef() == rhs.trackRef() && packRef() == rhs.packRef();
    }
...
public:
    ChnaChunk() {};
    ChnaChunk(std::initializer_list<AudioId> audioIds)
        : audioIds_(audioIds) {};
    ChnaChunk(std::vector<AudioId> audioIds)
        : audioIds_(audioIds) {};
...
public:
    ...
    uint64_t size() const override
    {
        return sizeof(bw64Size()) + sizeof(dataSize()) + sizeof(dummySize()) + sizeof(tableLength()) + table_.size() * 12;
    }
